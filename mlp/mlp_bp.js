...
// p5 js canvas size
var in1rbtn, in2rbtn;
// xor input radio button
var bpbtn, rstbtn;
// back-prop, weight reset button
var l1n1w1sl, l1n1w2sl, l1n2w1sl, l1n2w2sl, l2n1w1sl, l2n1w2sl;
// weight sliders
var neural_net;
var iter =0;
var training_interval;
// setInterval id for control training loop
...
var NetweightChart = function(options) {
    ...
    this.net_pos = options.net_pos || [1,0] // which neuron to track
    this.layout = {
        title: 'Cost on weights of Layer:'+this.net_pos[0]+' Neuron:'+(this.net_pos[1]+1),
        autosize: false,
        width: 380,
        height: 280,
...
NetweightChart.prototype.initplot = function() {
    // sync neural net weights to chart
        ...
        for (var j = 0; j < neuron_length + 1; j++) {
            ...
            if(i > 0) {
                for (var k = 0; k < this.net_outputs[i-1].length; k++) {
                    if (j === neuron_length) ww.push(this.bias);
                    else if(k === this.net_outputs[i-1].length -1) ww.push(this.bias);
                    else ww.push(Math.random()*2-1)
                }
            }
            ws.push(ww)
            bfouts.push(0)
...
NeuralNetwork.prototype.resetWeight = function () {
    this.net_weights = []
    // net_shape ex: [input,hidden #1,hidden #2,output] 
    for (var i = 0; i < this.net_length; i++) {
        var ws = []
...
NeuralNetwork.prototype.feedForwad = function (input_data) {
    ...
    // console.log('fp: '+JSON.stringify(this.net_weights))    
}
NeuralNetwork.prototype.backProp = function (train_data) {
    ...
    // console.log("bp step2: " + JSON.stringify(this.net_outputs) + " " +JSON.stringify(this.net_errors))    
    for (var i = this.net_weights.length - 1; i > 0; i--) {
        for (var cr = 0; cr < this.net_weights[i].length; cr++) {
            for (var pr = 0; pr < this.net_outputs[i-1].length; pr++) {
                this.net_weights[i][cr][pr] = this.net_weights[i][cr][pr] + (this.learning_rate * this.net_errors[i][cr] * this.net_outputs[i-1][pr])
                // if(!this.net_weights[i][cr][pr]) console.log('null error: '+i+' '+cr+' '+pr+' '+this.net_weights[i][cr][pr]+' '+ this.net_errors[i][cr] +' '+ this.net_outputs[i-1][pr])
            }
        }
    }
    // console.log('bp: '+JSON.stringify(this.net_weights))
}
...
var p5sketch = function(p) {
    p.setup = function() {
        ...
        bpbtn = new Button({p5:p, x: 750,y: 10, width: 280, height: 38, label: "Do Back Propagation 50 times",
            onClick: function() {
                ...
                if(!training_interval) training_interval = setInterval(function() {
                    ...
                    if(iter >= 50) { clearInterval(training_interval); training_interval = null; }
